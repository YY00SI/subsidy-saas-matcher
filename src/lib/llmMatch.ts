...
・資金調達（ファクタリング等）や通信環境整備は、事業の基盤改善という観点から必要性が認められる場合のみ適切なスコアを付与してください。
・【重要】リストにあるツールはすべて「IT・クラウド系ソフトウェア(SaaS)」または「無形の専門サービス」です。したがって、「機械装置等のハードウェア購入」「工場の改修」「新築・土木建築」「農業用機械の導入」のみを対象とする設備投資系・農業系・建設系の補助金に対しては、SaaSツールとの相性が皆無であるため、スコアは強制的に「0点」としてください。無理に合致理由を作らないでください。
